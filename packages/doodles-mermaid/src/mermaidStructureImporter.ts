...
import {
    ...
    type LayoutLink,
    type OrderHint,
} from "@benkalegin/doodles-layout";
import {createMermaidIdGenerator, mermaidSourceLines, parseMermaidLayoutHints} from "./mermaidImportUtils.js";
...
function toFlowchartKind(shape: "process" | "decision" | "terminator" | "input-output" | undefined): FlowchartNodeKind | undefined {
    switch (shape) {
        case "decision": return FlowchartNodeKind.Decision;
        case "terminator": return FlowchartNodeKind.Terminator;
        case "input-output": return FlowchartNodeKind.InputOutput;
        case "process": return FlowchartNodeKind.Process;
        default: return undefined;
    }
}

/**
 * Import a Mermaid class/flowchart diagram into the doodle structure format.
 */
...
function linkPortAlignments(direction: string | undefined, _flowchartMode: boolean): [PortAlignment, PortAlignment] {
    switch (direction) {
...
export async function importMermaidStructureDiagram(baseDiagram: Diagram, content: string, options?: ImportStructureOptions): Promise<Diagram> {
    ...
    const currentSubgraph = (): string => subgraphStack[subgraphStack.length - 1]!;
    ...
    function getOrCreateNode(
        ...
        if (nodeMap[normalizedName]) {
            ...
            if (subgraphStack.length > 0 && !nodeParents[nodeId]) {
                nodeParents[nodeId] = currentSubgraph();
            }
            return nodeId;
        }
    ...
    function setClassAnnotation(className: string, annotation: string): void {
        const nodeId = getOrCreateNode(className);
        const node = elements[nodeId] as NodeState;
        const normalized = normalizeClassAnnotation(annotation);
        if (normalized !== undefined) node.classAnnotation = normalized;
        nodes[nodeId].bounds.height = Math.max(nodes[nodeId].bounds.height, minimumClassNodeHeight(node));
    }
    ...
    for (const line of lines) {
        ...
        const classAssignMatch = flowchartMode
            ...
            : null;
        if (classAssignMatch) {
            const idsStr = classAssignMatch[1]!;
        ...
        if (inlineClassBlockMatch) {
            const className = inlineClassBlockMatch[1]!;
            const members = inlineClassBlockMatch[2]!;
        ...
        if (classBlockMatch) {
            ...
            const annotation = classBlockMatch[2];
            getOrCreateNode(className);
            if (annotation) {
        ...
        if (!flowchartMode && classMemberMatch) {
            const className = classMemberMatch[1]!;
            const memberText = classMemberMatch[2]!;
        ...
        if (!flowchartMode && classAnnotationMatch) {
            const annotation = classAnnotationMatch[1]!;
            const className = classAnnotationMatch[2]!;
            setClassAnnotation(className, annotation);
            continue;
        }
        ...
        if (c4NodeMatch) {
            const c4Type = c4NodeMatch[1]!;
            const id = c4NodeMatch[2]!;
        ...
        if (c4RelMatch) {
            const from = c4RelMatch[1]!;
            const to = c4RelMatch[2]!;
            const relLabel = c4RelMatch[3]!;
        ...
        if (nodeMatch) {
            ...
            const ioLabel = nodeMatch[2];
            const cylinderLabel = nodeMatch[3];
            const squareLabel = nodeMatch[4];
            const stadiumLabel = nodeMatch[5];
            const circleLabel = nodeMatch[6];
            const roundLabel = nodeMatch[7];
            const decisionLabel = nodeMatch[8];
        ...
        if (flowMatch) {
            ...
            const fio = flowMatch[2];
            const fcyl = flowMatch[3];
            const fsquare = flowMatch[4];
            const fstadium = flowMatch[5];
            const fcircle = flowMatch[6];
            const fround = flowMatch[7];
            const fdecision = flowMatch[8];
            const arrow = flowMatch[9]!;
            const edgeLabelBefore = flowMatch[10];
            const to = flowMatch[11]!;
            const tio = flowMatch[12];
            const tcyl = flowMatch[13];
            const tsquare = flowMatch[14];
            const tstadium = flowMatch[15];
            const tcircle = flowMatch[16];
            const tround = flowMatch[17];
            const tdecision = flowMatch[18];
            const edgeLabelAfter = flowMatch[19];
        ...
        if (classRelMatch) {
            const from = classRelMatch[1]!;
            const arrow = classRelMatch[2]!;
            const to = classRelMatch[3]!;
            const relLabel = classRelMatch[4];
            const fromNodeId = getOrCreateNode(from);
            const toNodeId = getOrCreateNode(to);
        ...
        if (classMatch) {
            ...
            const label = classMatch[2];
            const annotation = classMatch[3];
    ...
    // Preserve Mermaid declaration order across sibling targets.
    ...
    function applyStyleToTarget(mermaidId: string, props: StyleProps): void {
        ...
        const textColor = props.textColor ?? base.textColor;
        if (textColor !== undefined) next.textColor = textColor;
        target.colorSchema = next;
    }
